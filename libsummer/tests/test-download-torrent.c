...
static void
on_inner_fail (SummerDownload *dl, GError *error, gconstpointer user_data)
{
#if GLIB_CHECK_VERSION(2, 20, 0)
...
static void
inner_error (WebFixture *fix, gconstpointer data)
{
	loop = g_main_loop_new (NULL, TRUE);
	summer_download_set_default (g_get_tmp_dir (), NULL);
	SummerDownload *dl;
	SummerFeedData *feed = summer_feed_data_new ();
	SummerItemData *item = summer_feed_data_append_item (feed);
	gchar *url = g_strdup_printf ("http://127.0.0.1:%i/illegal/url", PORT);
	summer_item_data_append_downloadable (item,
		url,
		"application/x-bittorrent",
		0);
	...
	g_assert (SUMMER_IS_DOWNLOAD_TORRENT (dl));
	g_object_unref (feed);
	g_signal_connect (dl, "download-error", G_CALLBACK (on_inner_fail), NULL);
	g_signal_connect (dl, "download-complete", G_CALLBACK (on_completed_fail), NULL);
	GError *error = NULL;
	summer_download_start (dl, &error);
#if GLIB_CHECK_VERSION(2, 20, 0)
...
int main (int argc, char *argv[]) {
	...
	g_test_add ("/download-torrent/inner_error", WebFixture, 0, web_setup, inner_error, web_teardown);
#endif
